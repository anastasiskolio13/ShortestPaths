...
που απέχει τη μικρότερη απόσταση από την αφετηρία να την \emph{οριστικοποιεί} και να χαλαρώνει τις ακμές που εκκινούν από την κορυφή αυτή. Όταν μία κορυφή είνα οριστικοποιημένη, σημαίνει ότι έχει υπολογιστεί η τελική
...
Για να αποδείξουμε την ορθότητα του Dijkstra, Θα χρησιμοποιήσουμε \emph{αναλλοίωτη του βρόχου}. Για την συγκεκριμένη απόδειξη ορίζουμε ως $d(v, u)$, το μήκος του συντομότερου μονοπατιού
 από την $v$ στη $u$. 
\\ \\
\emph{Έναρξη.} Πρωτού αρχίσει να εκτελείται ο βρόχος της γραμμής 5, έχουμε ότι $S = \emptyset$ και επομένως η αναλλοίωτη συνθήκη ισχύει τετριμμένα. 
\\ \\
...
\begin{align*}
    d(s, v) < D[v].
\end{align*}
Συνδυαάζοντας τις τρεις τελευταίες ανισότητες παίρνουμε 
\begin{align*}
...
\begin{flushright}
    $\blacksquare$
\end{flushright}
Η αποδοτικότερη υλοποίηση του Dijkstra πραγματοποιείται με \emph{σωρό Fibonacci} και φράζεται από τα πάνω από την κλάση $\Theta(n \lg n + m)$. Υλοποιώντας τον αλγόριθμο με δυαδικό σωρό, μπορούμε με μία αρκετά απλότερη υλοποίηση 
